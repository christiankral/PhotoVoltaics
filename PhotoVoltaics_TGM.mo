package PhotoVoltaics_TGM
  ...
  model TGM_Comax_WeatherData_OneYear "One year based on real weather data; Comax modules at TGM building in Vienna, Austria"
    ...
    PhotoVoltaics.Components.SimplePlantSymmetric plant(useConstantIrradiance = false, moduleData = moduleData, npModule = npModule, nsModule = nsModule) annotation (
    ...
    WriteCSV writeCSV(fileName = Modelica.Utilities.Files.loadResource("modelica://PhotoVoltaics/TGM_Comax_WeatherData_OneYear_energy.csv"), inputQuantity = "energy(J)") annotation (
...
java -jar .../Buildings/Resources/bin/ConvertWeatherData.jar .../PhotoVoltaics/Resources/WeatherData/AUT_Vienna.Schwechat.110360_IWEC.epw
  ...
  model TGM_Comax_Analytical_OneYear "Analytical sun model over the year 2016; Comax modules at TGM building in Vienna, Austria"
    extends Modelica.Icons.Example;
    parameter Integer nsModule = 6 "Number of series connected modules";
    parameter Integer npModule = 1 "Number of parallel connected modules";
    PhotoVoltaics.Components.SimplePlantSymmetric plant(useConstantIrradiance = false, moduleData = moduleData, npModule = npModule, nsModule = nsModule) annotation (
    ...
    PhotoVoltaics.Sources.Irradiance.Irradiance irradiance(startDay = 1, startMonth = 1) annotation (
      Placement(visible = true, transformation(origin = {-90, -10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    WriteCSV writeCSV(fileName = Modelica.Utilities.Files.loadResource("modelica://PhotoVoltaics/TGM_Comax_Analytical_OneYear_energy.csv"), inputQuantity = "energy(J)") annotation (
      Placement(transformation(extent = {{-80, -70}, {-100, -50}})));
  equation
    connect(ground.p, plant.n) annotation (
      Line(points = {{-50, -30}, {-50, -20}}, color = {0, 0, 255}));
    ...
    connect(writeCSV.u, integrator.y) annotation (
      Line(points = {{-78, -60}, {-71, -60}}, color = {0, 0, 127}));
    annotation (
      experiment(StopTime = 3.1536e+07, Interval = 60, Tolerance = 1e-08, __Dymola_Algorithm = "Rkfix4", __OpenModelica_simulationFlags(jacobian = "coloredNumerical", s = "dassl", lv = "LOG_STATS")),
      Documentation(revisions = "<html>
</html>", info = "<html>
<p>This example is based on weather data taken from 
<a href=\"https://energyplus.net/weather-location/europe_wmo_region_6/AUT//AUT_Vienna.Schwechat.110360_IWEC\">https://energyplus.net</a>. 
...
java -jar .../Buildings/Resources/bin/ConvertWeatherData.jar .../PhotoVoltaics/Resources/WeatherData/AUT_Vienna.Schwechat.110360_IWEC.epw
  ...
  model TGM_Comax_Measurement_20160629 "Measured irradiance model on 2016-06-29; Comax modules at TGM building in Vienna, Austria"
    ...
    parameter Integer nsModule = 6 "Number of series connected modules";
    parameter Integer npModule = 1 "Number of parallel connected modules";
    parameter String fileName = Modelica.Utilities.Files.loadResource("modelica://PhotoVoltaics/Resources/WeatherData/TGM_20160629.txt") "File name";
    Modelica.SIunits.Power powerComax = combiTimeTable.y[2] "Measured power of Comax modules";
    PhotoVoltaics.Components.SimplePlantSymmetric plant(useConstantIrradiance = false, moduleData = moduleData, npModule = npModule, nsModule = nsModule) annotation (
      Placement(visible = true, transformation(origin = {-50, -10}, extent = {{10, -10}, {-10, 10}}, rotation = 90)));
    Modelica.Electrical.Analog.Basic.Ground ground annotation (
      Placement(visible = true, transformation(origin = {-50, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    Modelica.Blocks.Continuous.Integrator integrator(y(unit = "J")) annotation (
      Placement(transformation(extent = {{-50, -70}, {-70, -50}})));
    WriteCSV writeCSV(                                                                                                                    samplePeriod = 60, inputQuantity = "power(W)",
      fileName=Modelica.Utilities.Files.loadResource("modelica://PhotoVoltaics/TGM_Comax_Measurement_20160629power.csv"))                                                                annotation (
      Placement(transformation(extent = {{-10, -100}, {10, -80}})));
    Modelica.Blocks.Sources.CombiTimeTable combiTimeTable(
      tableOnFile=true,
      tableName="irradiance_power",
      fileName=fileName,
      columns=2:4)                                        annotation (Placement(transformation(extent={{-100,-20},{-80,0}})));
  equation
    connect(ground.p, plant.n) annotation (
      Line(points = {{-50, -30}, {-50, -20}}, color = {0, 0, 255}));
    connect(mpTracker.vRef, converter.vDCRef) annotation (
    ...
    connect(writeCSV.u, powerSensor.power) annotation (
      Line(points = {{-12, -90}, {-28, -90}, {-28, -1}}, color = {0, 0, 127}));
    connect(combiTimeTable.y[1], plant.variableIrradiance) annotation (Line(points={{-79,-10},{-62,-10}}, color={0,0,127}));
    annotation (
      experiment(StopTime = 86400, Interval = 60, Tolerance = 1e-08, __Dymola_Algorithm = "Rkfix4"),
      Documentation(revisions = "<html>
</html>", info = "<html>
...
java -jar .../Buildings/Resources/bin/ConvertWeatherData.jar .../PhotoVoltaics/Resources/WeatherData/AUT_Vienna.Schwechat.110360_IWEC.epw
  ...
  model TGM_Trina_WeatherData_OneYear "One year based on real weather data; Trina modules at TGM building in Vienna, Austria"
    ...
    Modelica.Blocks.Math.Add G "Total irradiation on tilted surface" annotation (
      Placement(visible = true, transformation(extent = {{-20, 50}, {0, 70}}, rotation = 0)));
    PhotoVoltaics.Components.SimplePlantSymmetric plant(moduleData = moduleData, npModule = npModule, nsModule = nsModule, useConstantIrradiance = false) annotation (
      Placement(visible = true, transformation(origin = {-50, -10}, extent = {{10, -10}, {-10, 10}}, rotation = 90)));
    Modelica.Electrical.Analog.Basic.Ground ground annotation (
      Placement(visible = true, transformation(origin = {-50, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(computeWetBulbTemperature = false, filNam = fileName) annotation (
    ...
    Modelica.Blocks.Continuous.Integrator integrator(y(unit = "J")) annotation (
      Placement(transformation(extent = {{-50, -70}, {-70, -50}})));
    PhotoVoltaics_TGM.WriteCSV writeCSV(fileName = Modelica.Utilities.Files.loadResource("modelica://PhotoVoltaics/TGM_Trina_WeatherData_OneYear_energy.csv"), inputQuantity = "energy(J)") annotation (
      Placement(transformation(extent = {{-80, -70}, {-100, -50}})));
...
java -jar .../Buildings/Resources/bin/ConvertWeatherData.jar .../PhotoVoltaics/Resources/WeatherData/AUT_Vienna.Schwechat.110360_IWEC.epw
  ...
  model TGM_Trina_Analytical_OneYear "Analytical sun model over the year 2016; Trina modules at TGM building in Vienna, Austria"
    extends Modelica.Icons.Example;
    parameter Integer nsModule = 8 "Number of series connected modules";
    parameter Integer npModule = 1 "Number of parallel connected modules";
    PhotoVoltaics.Components.SimplePlantSymmetric plant(moduleData = moduleData, npModule = npModule, nsModule = nsModule, useConstantIrradiance = false) annotation (
    ...
    PhotoVoltaics.Sources.Irradiance.Irradiance irradiance(startDay = 1, startMonth = 1) annotation (
      Placement(visible = true, transformation(origin = {-90, -10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    PhotoVoltaics_TGM.WriteCSV writeCSV(fileName = Modelica.Utilities.Files.loadResource("modelica://PhotoVoltaics/TGM_Trina_Analytical_OneYear_energy.csv"), inputQuantity = "energy(J)") annotation (
      Placement(transformation(extent = {{-80, -70}, {-100, -50}})));
    parameter PhotoVoltaics.Records.TSM_230_PC05 moduleData annotation (
      Placement(visible = true, transformation(origin = {70, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
...
java -jar .../Buildings/Resources/bin/ConvertWeatherData.jar .../PhotoVoltaics/Resources/WeatherData/AUT_Vienna.Schwechat.110360_IWEC.epw
  ...
  end TGM_Trina_Analytical_OneYear;
  ...
  model TGM_Trina_Measurement_20160629 "Measured irradiance model on 2016-06-29; Trina modules at TGM building in Vienna, Austria"
    ...
    Modelica.SIunits.Power powerTrina = combiTimeTable.y[2] "Measured power of Trina modules";
    PhotoVoltaics.Components.SimplePlantSymmetric plant(moduleData = moduleData, npModule = npModule, nsModule = nsModule, useConstantIrradiance = false,
    ...
    PhotoVoltaics_TGM.WriteCSV writeCSV(                                                                                                                    samplePeriod = 60, inputQuantity = "power(W)",
      fileName=Modelica.Utilities.Files.loadResource("modelica://PhotoVoltaics/TGM_Trina_Measurement_20160808_power.csv"))                                                                                 annotation (
      Placement(transformation(extent = {{-10, -100}, {10, -80}})));
    parameter PhotoVoltaics.Records.TSM_230_PC05 moduleData annotation (
      Placement(visible = true, transformation(origin = {70, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
